...
public class metodo_euler extends Dao {

    public double[] x;
    public double[] y;
    public datos euler = new datos();
    ...
    public void ingreso() {
        ...
        euler.setFuncion(text("INGRECE FUNCION: F(X,Y)="));
        euler.setN((int) Math.round(((r_final - euler.getX1()) / euler.getH()) + 1));
    }

    private String format(String funcion, double x, double y) {
        DecimalFormat format = new DecimalFormat("###0.0#####");
        return funcion.replace("x", c.purple("(") + format.format(x) + c.purple(")")).replace("y", c.purple("(") + format.format(y) + c.purple(")"));
    }
    @Override
    public void ejecucion_y() {
        ...
        for (int i = 1; i < euler.getN(); i++) {
            y[i] = y[i - 1] + (euler.getH() * (funcion(euler.getFuncion(), x[i - 1], y[i - 1])));
        }
    }
    ...
    public void salidita(double h,String fa,double x1,double y1,double N,double []x){
        ...
        for (int i = 1;  i<N ; i++) {
            ...
            y[i] = y[i - 1] + (euler.getH() * (funcion(euler.getFuncion(), xs, ys)));
