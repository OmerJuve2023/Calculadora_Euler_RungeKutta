...
public class metodo_runge_kutta extends Dao {
    ...
    public void ingreso() {
        ...
        runge.setFunction(text("INGRECE FUNCION: F(X,Y)="));
    ...
    public void ejecucion_y() {
        ...
        for (int i = 1; i < runge.getN(); i++) {
            double k1 = funcion(runge.getFunction(), x[i - 1], y[i - 1]);
            double k2 = funcion(runge.getFunction(), x[i - 1] + runge.getH() / 2, y[i - 1] + (runge.getH() / 2) * k1);
            double k3 = funcion(runge.getFunction(), x[i - 1] + runge.getH() / 2, y[i - 1] + (runge.getH() / 2) * k2);
            double k4 = funcion(runge.getFunction(), x[i - 1] + runge.getH(), y[i - 1] + (runge.getH() * k3));
